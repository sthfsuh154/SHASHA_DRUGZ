...
class TeleAPI:
    ...
    def _resolve_ext(self, file, mime_map: dict, fallback: str) -> str:
        ...
        try:
            ...
            if mime:
                ...
                if guessed: return guessed.lstrip(".").lower()
        except Exception: pass
        return fallback
    ...
    def is_video_document(self, file) -> bool:
        try:
            ...
            if "." in name:
                ext = name.rsplit(".", 1)[-1].lower().strip()
                if ext in DOCUMENT_VIDEO_EXTS: return True
        except Exception: pass
        try:
            mime = (file.mime_type or "").lower().strip()
            if mime in VIDEO_MIME_MAP or mime.startswith("video/"): return True
        except Exception: pass
        return False
    ...
    def is_audio_document(self, file) -> bool:
        try:
            ...
            if "." in name:
                ext = name.rsplit(".", 1)[-1].lower().strip()
                if ext in DOCUMENT_AUDIO_EXTS: return True
        except Exception: pass
        try:
            mime = (file.mime_type or "").lower().strip()
            if mime in AUDIO_MIME_MAP or mime.startswith("audio/"): return True
        except Exception: pass
        return False
    ...
    async def get_filename(self, file, audio: Union[bool, str] = None):
        try:
            ...
            if file_name is None: file_name = "ᴛᴇʟᴇɢʀᴀᴍ ᴀᴜᴅɪᴏ" if audio else "ᴛᴇʟᴇɢʀᴀᴍ ᴠɪᴅᴇᴏ"
        except Exception:
            file_name = "ᴛᴇʟᴇɢʀᴀᴍ ᴀᴜᴅɪᴏ" if audio else "ᴛᴇʟᴇɢʀᴀᴍ ᴠɪᴅᴇᴏ"
        return file_name

    async def get_duration(self, filex, file_path):
        try: dur = seconds_to_min(filex.duration)
        except Exception:
            try:
    ...
    async def get_filepath(self, audio: Union[bool, str] = None, video: Union[bool, str] = None):
        ...
        if audio:
            if isinstance(audio, Voice): ext = "ogg"
            else: ext = self._resolve_ext(audio, AUDIO_MIME_MAP, "mp3")
    ...
    async def download(self, _, message, mystic, fname):
        ...
        # FIX: Ensure a stale source file doesn't trigger cache overrides by completely trashing existing files
        ...
        async def down_load():
            async def progress(current, total):
                ...
                for counter in range(7):
                    ...
                    if low < percentage <= high:
                        if high == check:
                            try:
                                await mystic.edit_text(text=_["tg_1"].format(app.mention, total_size, completed_size, percentage, speed, eta), reply_markup=upl)
                                checker[counter] = 100
                            except Exception: pass
            ...
            try:
                ...
                try: elapsed = get_readable_time(int(time.time()) - int(speed_counter[message.id]))
                except Exception: elapsed = "0 sᴇᴄᴏɴᴅs"
