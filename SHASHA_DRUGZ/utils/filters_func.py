...
from SHASHA_DRUGZ.utils.msg_types import button_markdown_parser
from SHASHA_DRUGZ.utils.notes_func import NoteFillings
...
async def SendFilterMessage(
    ...
    # Map data_type → coroutine factory so we avoid repeating try/except
    async def _send():
        if data_type == 1:
    ...
    try:
        ...
        # Respect Telegram's flood wait and retry once
        ...
        # Bot hasn't cached this peer yet — silently skip.
        # The peer will be cached once the bot receives a message from that chat.
        pass
    except (ChatWriteForbidden, UserIsBlocked, InputUserDeactivated):
        # Bot was kicked / blocked — nothing we can do
        pass
    except RPCError:
...
def get_text_reason(message: Message) -> str:
    """Returns text and the reason from the user's arguments."""
