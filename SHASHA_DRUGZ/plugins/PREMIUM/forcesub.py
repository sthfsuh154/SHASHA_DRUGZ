...
async def admin_mute_in_vc(client: Client, chat_id: int, user_id: int) -> bool:
    try:
        ...
        print(f"[ғ-sᴜʙ ᴠᴄ ᴀᴅᴍɪɴᴍᴜᴛᴇ ᴇʀʀᴏʀ] chat={chat_id} user={user_id}: {e}")
        return False
...
async def set_forcesub(client: Client, message: Message):
    ...
    if not await is_owner_or_sudo(client, chat_id, user_id):
        return await message.reply_text(
            "🚫 **ᴘᴇʀᴍɪssɪᴏɴ ᴅᴇɴɪᴇᴅ!**\n\n"
            "ᴏɴʟʏ ᴛʜᴇ **ᴄʜᴀᴛ ᴏᴡɴᴇʀ** ᴏʀ **sᴜᴅᴏᴇʀs** ᴄᴀɴ ᴍᴀɴᴀɢᴇ ғᴏʀᴄᴇ sᴜʙsᴄʀɪᴘᴛɪᴏɴ.",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("❌ ᴄʟᴏsᴇ", callback_data=f"fsub_close_{user_id}")
            ]])
        )
    ...
    if args and args[0].lower() in ["off", "disable"]:
        if forcesub_collection.find_one({"chat_id": chat_id}):
            ...
            _cache_del(chat_id)
            return await message.reply_text(
                "✅ **ғᴏʀᴄᴇ sᴜʙsᴄʀɪᴘᴛɪᴏɴ ᴅɪsᴀʙʟᴇᴅ!**\n\n"
                "• ᴄʜᴀᴛ ᴍᴇssᴀɢᴇs: **ᴏᴘᴇɴ ᴛᴏ ᴀʟʟ**\n"
                "• ᴠᴏɪᴄᴇ ᴄʜᴀᴛ: **ᴏᴘᴇɴ ᴛᴏ ᴀʟʟ**",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("❌ ᴄʟᴏsᴇ", callback_data=f"fsub_close_{user_id}")
                ]])
            )
        return await message.reply_text(
            "ℹ️ ғᴏʀᴄᴇ sᴜʙsᴄʀɪᴘᴛɪᴏɴ ɪs ᴀʟʀᴇᴀᴅʏ **ᴅɪsᴀʙʟᴇᴅ** ғᴏʀ ᴛʜɪs ᴄʜᴀᴛ.",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("❌ ᴄʟᴏsᴇ", callback_data=f"fsub_close_{user_id}")
            ]])
        )
    ...
    if args and args[0].lower() in ["status", "info"]:
        ...
        if not data:
            return await message.reply_text(
                "📊 **ғᴏʀᴄᴇ sᴜʙsᴄʀɪᴘᴛɪᴏɴ sᴛᴀᴛᴜs**\n\n"
                "🔴 **sᴛᴀᴛᴜs:** ᴅɪsᴀʙʟᴇᴅ\n\n"
                "Use `/fsub <group_id>` ᴛᴏ ᴇɴᴀʙʟᴇ.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("❌ ᴄʟᴏsᴇ", callback_data=f"fsub_close_{user_id}")
                ]])
            )
        ...
        buttons.append([InlineKeyboardButton("❌ ᴄʟᴏsᴇ", callback_data=f"fsub_close_{user_id}")])
        return await message.reply_text(
            f"📊 **ғᴏʀᴄᴇ sᴜʙsᴄʀɪᴘᴛɪᴏɴ sᴛᴀᴛᴜs**\n\n"
            f"🟢 **sᴛᴀᴛᴜs:** ᴀᴄᴛɪᴠᴇ\n"
            f"📢 **ʀᴇǫᴜɪʀᴇᴅ ᴄʜᴀᴛ:** {title}\n\n"
            f"🔒 **ᴇɴғᴏʀᴄᴇᴅ ᴏɴ:**\n"
            f"  • 💬 ᴄʜᴀᴛ ᴍᴇssᴀɢᴇs\n"
            f"  • 🎙️ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ ᴊᴏɪɴs",
    ...
    if not args:
        return await message.reply_text(
            ...
            "`/fsub status`\n\n"
            "━━━━━━━━━━━━━━━━━━\n"
            "📌 ᴡᴏʀᴋs ɪɴ ɢʀᴏᴜᴘs **ᴀɴᴅ** ᴄʜᴀɴɴᴇʟs\n"
            "🔒 ᴏɴʟʏ **ᴄʜᴀᴛ ᴏᴡɴᴇʀ** ᴄᴀɴ ᴄᴏɴғɪɢᴜʀᴇ\n"
            "🎙️ ᴇɴғᴏʀᴄᴇs ᴏɴ **ᴠᴄ ᴊᴏɪɴs** ᴛᴏᴏ",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("❌ ᴄʟᴏsᴇ", callback_data=f"fsub_close_{user_id}")
            ]])
        )
    ...
    try:
        ...
        return await message.reply_text(
            f"🚫 **ᴄʜᴀᴛ ɴᴏᴛ ғᴏᴜɴᴅ!**\n\nᴇʀʀᴏʀ: `{e}`\n\n"
            "ᴍᴀᴋᴇ sᴜʀᴇ ᴛʜᴇ ᴜsᴇʀɴᴀᴍᴇ/ɪᴅ ɪs ᴄᴏʀʀᴇᴄᴛ ᴀɴᴅ ᴛʜᴇ ʙᴏᴛ ɪs ᴀ ᴍᴇᴍʙᴇʀ.",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("❌ ᴄʟᴏsᴇ", callback_data=f"fsub_close_{user_id}")
            ]])
        )
    ...
    try:
        ...
        return await message.reply_text(
            "⚠️ **ʙᴏᴛ ɪs ɴᴏᴛ ᴀᴅᴍɪɴ ɪɴ ᴛʜᴇ ʀᴇǫᴜɪʀᴇᴅ ᴄʜᴀᴛ!**\n\n"
            "**sᴛᴇᴘs ᴛᴏ ғɪx:**\n"
            "1️⃣ ᴏᴘᴇɴ ᴛʜᴇ ʀᴇǫᴜɪʀᴇᴅ ɢʀᴏᴜᴘ/ᴄʜᴀɴɴᴇʟ\n"
            "2️⃣ ɢᴏ ᴛᴏ **ᴀᴅᴍɪɴs → ᴀᴅᴅ ᴀᴅᴍɪɴ**\n"
            "3️⃣ ᴀᴅᴅ ᴛʜɪs ʙᴏᴛ ᴀs **ᴀᴅᴍɪɴ**\n"
            "4️⃣ ʀᴇ-ʀᴜɴ `/fsub <id>`",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("❌ ᴄʟᴏsᴇ", callback_data=f"fsub_close_{user_id}")
            ]])
    ...
    await message.reply_text(
        f"🎉 **ғᴏʀᴄᴇ sᴜʙsᴄʀɪᴘᴛɪᴏɴ ᴇɴᴀʙʟᴇᴅ!**\n\n"
        f"📢 **ʀᴇǫᴜɪʀᴇᴅ ᴄʜᴀᴛ:** [{req_title}]({req_url or '#'})\n\n"
        f"🔒 **ɴᴏᴡ ᴇɴғᴏʀᴄɪɴɢ:**\n"
        f"  • 💬 ɴᴏɴ-ᴍᴇᴍʙᴇʀs → **ᴍᴜᴛᴇᴅ** ᴡʜᴇɴ ᴛʜᴇʏ ᴛʀʏ ᴛᴏ sᴇɴᴅ ᴀ ᴍᴇssᴀɢᴇ\n"
        f"  • 🎙️ ɴᴏɴ-ᴍᴇᴍʙᴇʀs → **ᴋɪᴄᴋᴇᴅ** ғʀᴏᴍ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ\n\n"
        f"⚡ ᴜsᴇʀs ᴡɪʟʟ ʙᴇ **ᴀᴜᴛᴏ-ᴜɴᴍᴜᴛᴇᴅ** ᴏɴᴄᴇ ᴛʜᴇʏ ᴊᴏɪɴ ᴛʜᴇ ʀᴇǫᴜɪʀᴇᴅ ᴄʜᴀᴛ.\n\n",
        reply_markup=InlineKeyboardMarkup(buttons),
        disable_web_page_preview=True
    )
...
async def cb_disable(client: Client, cq: CallbackQuery):
    ...
    if cq.from_user.id != auth_user and cq.from_user.id not in SUDOERS:
        return await cq.answer("⚠️ ᴏɴʟʏ ᴛʜᴇ ᴄᴏᴍᴍᴀɴᴅ ɪssᴜᴇʀ ᴄᴀɴ ᴜsᴇ ᴛʜɪs!", show_alert=True)
    forcesub_collection.delete_one({"chat_id": target_chat})
    _cache_del(target_chat)
    await cq.answer("✅ ғᴏʀᴄᴇ sᴜʙsᴄʀɪᴘᴛɪᴏɴ ᴅɪsᴀʙʟᴇᴅ!", show_alert=True)
    try:
        await cq.message.edit_text(
            "✅ **ғᴏʀᴄᴇ sᴜʙsᴄʀɪᴘᴛɪᴏɴ ᴅɪsᴀʙʟᴇᴅ.**\n\n"
            "ᴀʟʟ ᴍᴇᴍʙᴇʀs ᴄᴀɴ ɴᴏᴡ ғʀᴇᴇʟʏ ᴄʜᴀᴛ ᴀɴᴅ ᴊᴏɪɴ ᴠᴄ."
        )
    except Exception:
        pass
...
async def cb_close(client: Client, cq: CallbackQuery):
    ...
    if cq.from_user.id != auth_user and cq.from_user.id not in SUDOERS:
        return await cq.answer("⚠️ ᴛʜɪs ɪs ɴᴏᴛ ғᴏʀ ʏᴏᴜ!", show_alert=True)
    try:
...
async def on_user_join_chat(client: Client, update):
    try:
        ...
        buttons = []
        if req_url:
            buttons.append([InlineKeyboardButton(f"🔻𝐉ᴏɪɴ 𝐓ᴏ 𝐔ɴᴍᴜᴛᴇ🔻", url=req_url)])

        try:
            await client.send_message(
                chat_id,
                f"<blockquote>🔒 **ʜᴇʏ {new_member.user.mention}!**</blockquote>\n"
                f"<blockquote>ʏᴏᴜ ʜᴀᴠᴇ ʙᴇᴇɴ **ᴍᴜᴛᴇᴅ** ʙᴇᴄᴀᴜsᴇ ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀ ᴍᴇᴍʙᴇʀ ᴏғ **<a href='{req_url}'>{req_title}</a>**.</blockquote>\n"
                f"<blockquote>**ᴛᴏ ɢᴇᴛ ᴜɴᴍᴜᴛᴇᴅ:**\n"
                f"ᴊᴏɪɴ \n\n**<a href='{req_url}'>{req_title}</a>** ᴜsɪɴɢ ᴛʜᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ</blockquote>\n"
                f"<blockquote>⚡ ʏᴏᴜ'ʟʟ ʙᴇ **ᴜɴᴍᴜᴛᴇᴅ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ** ᴏɴᴄᴇ ʏᴏᴜ ᴊᴏɪɴ!</blockquote>",
...
async def _handle_vc_violator(
    ...
    try:
        ...
        try:
            ...
            buttons_dm = []
            if req_url:
                buttons_dm.append([InlineKeyboardButton(f"🔻𝐉ᴏɪɴ 𝐓ᴏ 𝐔ɴᴍᴜᴛᴇ🔻", url=req_url)])
            if chat_username:
                buttons_dm.append([
                    InlineKeyboardButton("🎙️ 𝐑𝐞𝐣𝐨𝐢𝐧 𝐕𝐂", url=f"https://t.me/{chat_username}")
                ])
            await client.send_message(
                user_id,
                f"🚫 **𝐘𝐨𝐮 𝐰𝐞𝐫𝐞 𝐫𝐞𝐦𝐨𝐯𝐞𝐝 𝐟𝐫𝐨𝐦 𝐕𝐨𝐢𝐜𝐞 𝐂𝐡𝐚𝐭!**\n\n"
                f"**𝐂𝐡𝐚𝐭:** {chat_title}\n\n"
                f"𝐓𝐨 𝐣𝐨𝐢𝐧 𝐭𝐡𝐞 𝐕𝐂, 𝐲𝐨𝐮 𝐦𝐮𝐬𝐭 𝐛𝐞 𝐚 𝐦𝐞𝐦𝐛𝐞𝐫 𝐨𝐟 **{req_title}**.\n\n"
                f"**𝐒𝐭𝐞𝐩𝐬:**\n"
                f"1️⃣ 𝐉𝐨𝐢𝐧 **{req_title}**\n"
                f"2️⃣ 𝐑𝐞𝐭𝐮𝐫𝐧 𝐭𝐨 **{chat_title}**\n"
                f"3️⃣ 𝐑𝐞𝐣𝐨𝐢𝐧 𝐭𝐡𝐞 𝐕𝐨𝐢𝐜𝐞 𝐂𝐡𝐚𝐭",
        ...
        try:
            await message.reply_photo(
                ...
                caption=(
                    ...
                    f"𝐘𝐨𝐮 𝐧𝐞𝐞𝐝 𝐭𝐨 𝐣𝐨𝐢𝐧 **{req_title}** 𝐭𝐨 𝐬𝐞𝐧𝐝 𝐦𝐞𝐬𝐬𝐚𝐠𝐞𝐬 𝐡𝐞𝐫𝐞.\n\n"
                    f"1️⃣ 𝐂𝐥𝐢𝐜𝐤 **'ᴊᴏɪɴ'** ʙᴇʟᴏᴡ\n\n"
