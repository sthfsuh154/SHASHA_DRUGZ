...
# ✅ Fixed: Changed to real chat id
MESSAGE_DUMP = -1001735663878
FILLERS = {}
...
async def clog(
    ...
    if cq:
        data += str(cq)
        data += "\n\n\n\n"
    # ✅ Fixed: wrap send_message in try-except to avoid crashes
    try:
...
async def get_top_animes(gnr: str, page, user):
    ...
    # ✅ Fixed: handle API failure or missing data
    ...
    if len(result["data"]["Page"]["media"]) == 0:
        ...
        result = await return_json_senpai(query, vars_, auth=False, user=user)
        if not result or "data" not in result or len(result["data"]["Page"]["media"]) == 0:
...
async def anime_cmd(client: Client, message: Message, mdata: dict):
    ...
    try:
        ...
        ufc = await get_user_from_channel(user)
        if ufc is not None:
            auser = ufc
        else:
            auser = user
...
async def page_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    if "-100" in str(user):
        auser = await get_user_from_channel(user)
    else:
        auser = user
...
async def favourites_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    q = cdata["data"].split("_")
    if "-100" in str(q[5]):
        auser = await get_user_from_channel(q[5])
    else:
        auser = q[5]
...
async def get_user_back_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    query = cdata["data"].split("_")[1]
    if "-100" in str(query):
        auser = await get_user_from_channel(query)
    else:
        auser = query
...
example: /fillers Detective Conan"""
    ...
    for i in list_:
        ...
        buttons.append([InlineKeyboardButton(i, callback_data=f"fill_{fl_js}_{user}")])
    await message.reply_text(
...
async def fillers_cmd_dup(client: app, message: Message):
