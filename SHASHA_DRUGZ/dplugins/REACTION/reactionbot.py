...
# Bare-command trigger words — watcher must skip these
_BARE_COMMANDS = {"reaction", "react"}
...
async def set_chat_reaction_enabled(chat_id: int, enabled: bool):
    ...
    await SETTINGS.update_one(
        {"_id": key}, {"$set": {"enabled": enabled}}, upsert=True
    )
...
# ==============================================================================
#                         SHARED SETTINGS PANEL SENDER
# ==============================================================================
async def _send_reaction_panel(client, message: Message):
    """Build and send the reaction settings panel. Used by both command handlers."""
...
# ── Slash / prefixed commands: /reaction  /react  !reaction  !react  etc. ──
@Client.on_message(
    filters.command(["reaction", "react"], prefixes=["/", "!", "%", ",", ".", "@", "#"])
)
async def react_command_prefixed(client, message: Message):
    await _send_reaction_panel(client, message)


# ── Bare text commands (no prefix): "reaction" or "react" as full message ──
# Uses regex so it only fires when the ENTIRE message is one of those words.
@Client.on_message(
    filters.regex(r"^(reaction|react)$", re.IGNORECASE)
    & (filters.group | filters.private)
)
async def react_command_bare(client, message: Message):
    await _send_reaction_panel(client, message)


# ── Callback buttons ──
@Client.on_callback_query(filters.regex("^react_"))
async def reaction_callback(client, query: CallbackQuery):
    ...
    try:
        if action.startswith("react_chat_on:"):
            ...
            await query.edit_message_text("✅ **Auto-Reactions Enabled for this chat!**")

        elif action.startswith("react_chat_off:"):
            chat_id = int(action.split(":")[1])
            await set_chat_reaction_enabled(chat_id, False)
            await query.edit_message_text(
                "❌ **Auto-Reactions Disabled for this chat!**\n"
            ...
            await query.edit_message_text("🧹 **Settings Cleared.** Using global default.")
...
@Client.on_message(filters.command("addreact", prefixes=["/"]))
async def add_react(client, message: Message):
    ...
    if resolved_id:
        ...
        await COLLECTION.update_one(
            {"name": id_key}, {"$set": {"name": id_key}}, upsert=True
        )
...
@Client.on_message(filters.command("delreact", prefixes=["/"]))
...
def message_words(text: str) -> Set[str]:
    return set(m.group(1).lower() for m in WORD_RE.finditer(text))


# ── FIX 1: group=99 — runs AFTER all other modules, so StopPropagation
#           from earlier groups can never kill this handler.
# ── FIX 2: bare "reaction"/"react" words are skipped so the watcher
#           doesn't auto-react when someone is invoking the settings command.
@Client.on_message(
    (filters.text | filters.caption) & (filters.group | filters.private),
    group=99,
)
async def reaction_watcher(client, message: Message):
    ...
    # Skip all command-like messages (prefixed or bare command words)
    if text.startswith(("/", "!", ".", "#", "$", "%", ",", "@")):
        return
    if text.lower() in _BARE_COMMANDS:
        return
    ...
    # ── Analyse entities (@ mentions / text_mention) ──
...
__mod_name__ = "H_B_10"
__help__ = """
🔻 /reaction | /react | reaction | react ➠ ᴏᴘᴇɴꜱ ᴀᴜᴛᴏ-ʀᴇᴀᴄᴛɪᴏɴ ꜱᴇᴛᴛɪɴɢꜱ ꜰᴏʀ ᴛʜᴇ ᴄʜᴀᴛ (ᴀᴅᴍɪɴꜱ ᴏɴʟʏ).
