...
from SHASHA_DRUGZ.misc import HAPP, SUDOERS, XCB
...
@Client.on_callback_query(filters.regex(r"refresh_logs"))
...
@Client.on_message(filters.command(["clog", "clogs", "carbonlog", "carbonlogs"], prefixes=["/", "!", "%", ",", "", ".", "@", "#"]) & SUDOERS)
@language
...
@Client.on_message(filters.command(["getlog", "logs", "getlogs"], prefixes=["/", "!", "%", ",", "", ".", "@", "#"]) & SUDOERS)
@language
async def log_(client, message, _):
    try:
...
@Client.on_message(filters.command(["update", "gitpull"], prefixes=["/", "!", "%", ",", "", ".", "@", "#"]) & SUDOERS)
@language
...
@Client.on_message(filters.command(["restart"]) & SUDOERS)
async def restart_(client: Client, message):
...
__mod_name__ = "H_B_59"
__help__ = """
🔻 /getlog or /logs or /getlogs ➠ sᴇɴᴅ ᴛʜᴇ ʟᴏɢ ғɪʟᴇ (log.txt) ᴀs ᴀ ᴅᴏᴄᴜᴍᴇɴᴛ.
🔻 /update or /gitpull ➠ ᴜᴘᴅᴀᴛᴇ ᴛʜᴇ ʙᴏᴛ ғʀᴏᴍ ᴛʜᴇ ᴜᴘsᴛʀᴇᴀᴍ ʀᴇᴘᴏsɪᴛᴏʀʏ.
🔻 /restart ➠ ʀᴇsᴛᴀʀᴛ ᴛʜᴇ ʙᴏᴛ ᴀɴᴅ ᴄʟᴇᴀɴ ᴛᴇᴍᴘᴏʀᴀʀʏ ғɪʟᴇs.
"""
MOD_TYPE = "SUDO"
MOD_NAME = "Restart"
MOD_PRICE = "30"
