...
async def play_commnd(
    ...
    if audio_telegram:
        ...
        if await Telegram.download(_, message, mystic, file_path):
            ...
            details = {
                "title": file_name, "link": message_link,
                "path": file_path, "dur": dur,
            }
            try:
                await stream(
    ...
    if str(playmode) == "Direct":
        ...
        if plist_type:
            ...
            if slider:
                ...
                await message.reply_photo(photo=thumb, caption=cap, reply_markup=InlineKeyboardMarkup(buttons))
...
# =============================================================================
#  STREAM DISPATCHER  —  uses _safe_send_photo / _safe_send_message
#  so deployed bots send via their own Pyrogram client, not app (main bot)
# =============================================================================
async def stream(
    _,
    mystic,
            ...
            if await is_active_chat(chat_id):
                ...
                run = await _safe_send_photo(
                    ...
                    caption=_["stream_1"].format(
                        ...
                        title[:11], duration_min, user_name,
                    ),
                    reply_markup=InlineKeyboardMarkup(button),
                )
                db[chat_id][0]["mystic"] = run
    ...
    # ── INSTAGRAM ─────────────────────────────────────────────────────────────
        ...
        try:
            ig_result, file_path = await Instagram.download(ig_url, mystic, video=video, audio=not video)
        except Exception:
            raise AssistantErr(_["play_14"])
    ...
    # ── SOUNDCLOUD ───────────────────────────────────────────────────────────
        ...
        if await is_active_chat(chat_id):
            ...
            run = await _safe_send_photo(
                ...
                caption=_["stream_1"].format(config.SUPPORT_CHAT, title[:11], duration_min, user_name),
                reply_markup=InlineKeyboardMarkup(button),
            )
            db[chat_id][0]["mystic"] = run
            db[chat_id][0]["markup"] = "tg"
    ...
    # ── TELEGRAM ─────────────────────────────────────────────────────────────
        ...
        if await is_active_chat(chat_id):
            ...
            await put_queue(
                chat_id, original_chat_id, file_path, title, duration_min,
                user_name, streamtype, user_id, "video" if video else "audio", forceplay=forceplay,
            )
            if video:
